...
Create Authorized Session
    [Arguments]    ${token}
    [Documentation]    Create an API session with all required headers
...
Send Bundles Request
    [Documentation]    POST to /api/v2/bundles with bundle_ids
...
Validate Get Bundles V2 API Returns 200
    [Documentation]    Get fresh token, then verify /api/v2/bundles returns 200 OK
